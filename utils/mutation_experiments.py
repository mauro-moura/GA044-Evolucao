        ...
        for v in v_list:
            ...
            for k, p0 in enumerate(p0_list):
                ...
                for i in t:
                    ...
                    if int(p * pop_size) == int(p_t[i-1] * pop_size) and not estag_gens.get(dict_key, None):
                        print(f"Estagnation in Gen {i}\n p: {p} q: {q}")
                        estag_gens[dict_key] = i
                    ...
                    # Verificar perda genética
                    if int(pq * pop_size) == 0 and not gen_loss.get(dict_key, None):
                        print(f"Genetic Loss in Gen {i}") 
                        gen_loss[dict_key] = i

                    # Adicionar os valores calculados à lista
                    p_t.append(p)
                    q_t.append(q)
                    p2_t.append(p2)
                    q2_t.append(q2)
